...
module.exports.engine = function() {
  const resolve = (views, name) => {
    if (views instanceof Map) {
      return views.get(name) || views.get(name + view.extname);
    }
    return views[name] || views[name + view.extname];
  };
  ...
  const engine = {
    ...
    compile(view, options = {}) {
      if (options.recompile !== true) view.fn = () => {};
      return view;
    },
    async render(view, locals, options = {}) {
      engine.renderSync(view, locals, options);
      return view;
    },
    renderSync(view, locals, options = {}) {
      const { layouts, resolveLayout = resolve, transform } = options;
      render(view, layouts, { resolveLayout, transform });
      return view;
    }
  };
