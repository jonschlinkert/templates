...
module.exports = function(proto) {
  ...
  proto.compile = function(view, locals, isAsync) {
    ...
    if (typeof engine === 'undefined') {
      throw this.formatError('compile', 'engine', ext);
    }

    engine.options.engineName = engine.options.name;
    delete engine.options.name;

    var settings = utils.extend({}, locals.settings, engine.options);
    ...
    settings = utils.extend({}, ctx, locals, settings);
  ...
  proto.render = function(view, locals, cb) {
    ...
    var ctx = utils.extend({}, this.cache.data, view.locals, locals);
    ...
    // get the engine
    var ext = utils.resolveEngine(view, ctx, opts);
    var engine = this.getEngine(ext);

    if (!engine) {
      return cb(this.formatError('render', 'engine', ext));
    }
