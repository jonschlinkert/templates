...
module.exports = function(proto) {
  ...
  proto.compile = function(view, locals, isAsync) {
    ...
    settings = utils.merge({}, settings, this.mergePartialsSync(settings));
  ...
  proto.compileAsync = function(view, locals, isAsync, cb) {
    ...
    this.handle('preCompile', view, function(err, file) {
      ...
      if (typeof file.engineFn === 'function') {
        file.engineFn();
      }

      // determine the name of the engine to use
      var ext = utils.resolveEngine(file, locals, app.options);
      ...
      app.applyLayoutAsync(file, function(err, view) {
        ...
        // Bind context to helpers before passing to the engine.
        app.bindHelpers(view, locals, ctx, (locals.async = !!isAsync));

        app.mergePartials(settings, function(err, partials) {
          if (err) return cb(err);

          // shallow clone the context and locals
          var mergedSettings = utils.merge({}, settings, ctx, locals, partials);
