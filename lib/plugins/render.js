...
module.exports = function(proto) {
  ...
  proto.compile = function(view, locals, isAsync) {
    ...
    settings = utils.merge({}, ctx, settings, this.mergePartials(settings));
  ...
  proto.compileAsync = function(view, locals, isAsync, cb) {
    ...
    this.handle('preCompile', view, function(err, file) {
      ...
      // get engine options (settings)
      var settings = utils.merge({}, ctx.settings, engine.options);
      ...
      app.applyLayoutAsync(file, function(err, view) {
        ...
        app.mergePartialsAsync(settings, function(err, partials) {
          if (err) return cb(err);

          // shallow clone the context and settings
          var mergedSettings = utils.merge({}, settings, ctx, partials);
