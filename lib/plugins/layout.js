...
module.exports = function(proto) {
  ...
  proto.applyLayout = function(view) {
    ...
    // trigger getter
    if (utils.hasBlock(view)) {
      view.ast;
    }

    // get the name of the first layout
    var name = this.resolveLayout(view) || view.extends;
    var app = this;
  ...
  proto.applyLayoutAsync = function(view, cb) {
    ...
    this.handleOnce('preLayout', view, function(err, file) {
      ...
      // trigger getter
      if (utils.hasBlock(file)) {
        file.ast;
      }

      // get the name of the first layout to use
      var name = app.resolveLayout(file) || file.extends;
      var opts = { normalizeWhitespace: false };
