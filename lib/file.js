...
class File {
  ...
  set cwd(val) {
    ...
    this._cwd = path.resolve(normalize(val));
  }
  get cwd() {
    return this._cwd;
  }
  ...
  set base(val) {
    ...
    const base = path.resolve(normalize(val));
    ...
    var filepath = this.history.length ? this.path : null;
    var relative = filepath ? this.relative : null;
  ...
  set path(filepath) {
    ...
    let val = path.resolve(normalize(filepath));
    if (val !== this.path) {
...
function isBuffer(val) {
  if (isObject(val) && val.constructor && typeof val.constructor.isBuffer === 'function') {
