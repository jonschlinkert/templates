...
module.exports = function(app, options) {
  ...
  app.asyncHelper('view', function fn(name, collectionName, options, cb) {
    ...
    options = utils.merge({}, options, options.hash);

    var view = app.find(name, collectionName);
    if (view && options.render === true) {
      delete options.render;

      var ctx = utils.merge({}, options, this.context);
      return this.app.render(view, ctx, function(err, res) {
        if (err) return cb(err);
        cb(null, res.content);
      });
    }
    cb(null, view ? view.content : '');
  });
