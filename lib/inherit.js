...
function getDescriptor(provider, key) {
  return Object.getOwnPropertyDescriptor(provider, key);
}

function copy(receiver, provider, key) {
  var val = getDescriptor(provider, key);
  if (val) define(receiver, key, val);
}
...
function inherit(receiver, provider) {
  ...
  if (receiver.prototype && provider.prototype) {
    ...
    define(receiver, '__super__', provider.prototype);
  }
}
