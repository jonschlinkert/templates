...
List.prototype.init = function(opts) {
  ...
  if (Array.isArray(opts) || opts instanceof List) {
    this.options = opts.options;
    this.addList(opts);

  } else if (opts instanceof this.Views) {
    this.options = opts.options;
...
/**
 * Similar to `setItem`, adds an item to the list but also fires an
 * event and iterates over the item `queue` to load items from the
 * `addItem` event listener.  An item may be an instance of `Item`,
