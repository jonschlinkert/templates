...
class Common extends Emitter {
  constructor(options) {
    ...
    this.router = new utils.Router({ handlers: this.options.handlers });
  ...
  engine(name, engine) {
    const dot = str => str && str[0] !== '.' ? '.' + str : str;
    if (!name) name = this.options.engine;
    assert.equal(typeof name, 'string', 'expected engine name to be a string');
    if (!engine) {
      engine = this.engines.get(dot(name)) || this.engines.get(dot(this.options.engine));
      assert(engine, `engine "${name}" is not registered`);
      return engine;
    }
    this.engines.set(dot(name), engine);
  ...
  async renderLayout(view, options, fn) {
    ...
    const views = (this.kinds && this.kinds.layout) || opts.layouts;
  ...
  async compile(val, options) {
    ...
    if (view.engine === false) return view;
  ...
  async render(val, locals = {}, options = {}) {
    ...
    const defaults = { engine: view.engine, helpers: this.helpers, partials: kinds.partial };
...
function pick(obj, arr) {
  ...
  for (const key of arr) {
    if (obj[key]) {
      res[key] = obj[key];
      obj[key] = undefined; // dereference, then delete
      delete obj[key];
    }
  }
  return res;
