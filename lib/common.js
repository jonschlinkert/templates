...
const { define, isObject, Router, stripDot } = require('./utils');
...
class Common extends Emitter {
  ...
  renderLayout(view, options, fn) {
    ...
    if (this.options.renderLayout === false || view.renderLayout === false) {
      return view;
    }
    ...
    if (view.layout) {
      ...
      engine.renderSync.call(this, view, null, opts);
  ...
  compile(val, options = {}) {
    ...
    if (view.fn && view.recompile !== true && options.recompile !== true) return view;
  ...
  render(view, locals, options = {}) {
    ...
    // compile the view
    this.compile(view, engineOptions);
    ...
    engine.renderSync.call(this, view, context, engineOptions);
   ...
   * loaded unless specified by the user. This can slightly improve
  ...
  get router() {
    ...
    router = new Router({ handlers, sync });
    router.mixin(this);
