...
class Common extends Emitter {
  constructor(options) {
    ...
    this.router.on('handler', (name, handler) => {
      this[name] = handler.bind(this.router);
    });

    this.router.on('handle', (method, file, route) => {
  ...
  view(key, view = {}) {
    return this.toFile(key, view, this.View);
  }

  file(key, file = {}) {
    return this.toFile(key, file);
  }

  toFile(key, file, Ctor = this.File) {
    ...
    file = !Ctor.isValid(file) ? new Ctor(file) : file;
  ...
  async renderLayout(view, options) {
  ...
  async write(options) {
    ...
    return async(locals, fn) => {
      const data = Object.assign({}, locals);
      data.pager = this.paginate(list, options);
