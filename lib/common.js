...
class Common extends Emitter {
   ...
   *
   * ```js
   * const file = {%= type %}.file({ path: 'some-file.hbs', contents: Buffer.from('...') });
   * {%= type %}.compile(file)
  ...
  async compile(val, options = {}) {
    ...
    const name = first([file.engine, this.options.engine, options.engine]);
  ...
  prepareRender(file, locals = {}, options = {}) {
  ...
  //   assert(this.File.isFile(file), `expected a file: ${file}`);

  //   // exit early
  //   if (file.render === false || options.render === false || !file.engine) return file;
  ...
  //   // create context to use for rendering file.contents.
  //   const context = { ...data, ...locals };

  //   // compile the file
  //   await this.compile(file, engineOptions);
  ...
  //   const engine = context.engine = this.engine(file.engine);

  //   // actually render the file
  //   await engine.render.call(this, file, context, engineOptions);
  ...
  async render(val, locals, options) {
    ...
    const { opts, context } = this.prepareRender(file, locals, options);
    return this.renderFile(file, context, opts);
  }

  async renderFile(file, locals, options) {
    const engine = this.engine(file.engine);

    await this.compileFile(file, engine, options);
    await this.handle('preRender', file);
    await engine.render.call(this, file, locals, options);
