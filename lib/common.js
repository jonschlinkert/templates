...
class Common extends Emitter {
  ...
  engine(ext, engine) {
    ...
    if (!engine && !name && name !== false) {
      engine = this.engines.get(utils.dot(this.options.engine));
    }
  ...
  compile(val, options = {}) {
    ...
    const engine = this.engine(this.options.engine || view.engine);
  ...
  render(val, locals, options) {
    const view = this.get(val);
    assert(View.isView(view), `expected a view: ${val}`);
    ...
    return this.renderView(view, { ...data, ...locals }, opts);
  }
