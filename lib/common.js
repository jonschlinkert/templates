...
class Common extends Emitter {
  ...
  engine(name, engine, options) {
    ...
    engine = this.engines.get(utils.dot(name));
  ...
  renderLayoutSync(view, options, fn) {
  ...
  compileSync(val, options = {}) {
    ...
    this.renderLayoutSync(view, options);
  ...
  renderSync(val, locals, options) {
    const { view, opts, engine, context } = this.prepareRender(val, locals, options);
    this.handle('preRender', view);
    this.compileSync(view, opts);
