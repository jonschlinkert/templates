...
utils.hasBlock = function(file) {
  if (file.isNull()) {
    file.hasBlock = false;
  }
  if (typeof file.hasBlock !== 'boolean') {
    var blockRe = /\{%\s*(block|extends|layout)[^%]*%}/;
    file.hasBlock = blockRe.test(file.contents.toString());
  }
  return file.hasBlock;
};
