...
const { isBinaryFile, isBuffer, isObject, isStream, pathEndsWith } = require('./utils');
...
class View {
  constructor(view = {}) {
    ...
    this.history = view.history ? view.history.map(normalize) : [];
  ...
  pathEndsWith(substr, options) {
    if (pathEndsWith(this.path, substr, options)) return true;
    return pathEndsWith(this.history[0], substr, options);
  }
  ...
  set size(size) {
    throw new Error('view.size is a getter and may not be defined');
  }
  get size() {
    return this.stat ? this.stat.size : (this.contents ? this.contents.length : 0);
  }
