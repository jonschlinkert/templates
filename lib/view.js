...
View.prototype.compile = function (settings) {
  this.fn = utils.engine.compile(this.contents.toString(), settings);
  return this;
};
...
utils.define(View.prototype, 'content', {
  set: function(val) {
    ...
    if (!val && utils.isBuffer(this.contents)) {
      val = this.contents.toString();
    }
    this.contents = new Buffer(val);
    this.define('_content', val);
  },
  get: function() {
    if (!this._content && utils.isBuffer(this.contents)) {
      this._content = this.contents.toString();
    }
