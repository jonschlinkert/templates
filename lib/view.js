...
class View {
  constructor(view = {}) {
    assert(utils.isObject(view), 'expected view to be an object');
    this.type = 'view';
    this.history = [];
    this.cache = {};
    this.data = {};
    ...
    this.stat = view.stat;
    if (view.contents) this.contents = view.contents;
  ...
  reset() {
    this._contents = this._origContents;
  ...
  set contents(val) {
    ...
    if (val !== null && !utils.isBuffer(val) && !utils.isStream(val)) {
      throw new TypeError('expected view.contents to be a string, buffer, stream, or null');
    }
    this._contents = this._origContents = val;
  }
  get contents() {
  ...
  static isView(view) {
    return utils.isObject(view) && (view.isView === true || view._isVinyl === true);
  }
