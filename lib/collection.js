...
Collection.prototype.createGroup = function(level, namespace, group, options) {
  ...
  if (Array.isArray(group)) {
    this.createGroupPages(plural, namespace, group, options);
    return this;
  }
  ...
  keys.forEach(function(key) {
    if (Array.isArray(group[key])) {
      list[key] = this.createListPages(key, single, `${namespace}/${key}`, group[key], level, options);
      return;
    }
    list[key] = this.createListPage(key, single, `${namespace}/${key}`, group[key], level, options);
  }, this);
...
Collection.prototype.createListPage = function(name, single, namespace, group, level, options) {
  ...
  if (Array.isArray(group)) {
    item.data.items = group;
  } else {
    item.data[single] = name;
    item.data[options.names[level + 1]] = group;
