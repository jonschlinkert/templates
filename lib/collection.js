...
class Collection extends Common {
  ...
  groupBy(prop, options = {}) {
    ...
    for (const item of list) {
      let arr = typeof prop === 'function'
        ? prop(item)
        : get(item, prop) || get(item.data, prop);
  ...
  collect(prop, options = {}, fn) {
    ...
    const name = options.singular || prop;
    const group = this.groupBy(prop, options);
    ...
    for (const key of keys) {
      ...
      this.paginate({ ...options, list }, (page, i) => {
        ...
        if (i === 0) {
          page.path = `${lastSeg}/${key}/index.html`;
        } else {
          page.path = `${lastSeg}/${key}/page/${i + 1}/index.html`;
        }
