...
class File {
  ...
  [util.inspect.custom]() {
    const filepath = this.path && this.base ? `"${this.relative}"` : '';
    const inspect = [filepath.replace(/^"(?:\.\.\/)+volumes\/(\w+)/, '"$1:')];
  ...
  clone(options = {}) {
    ...
    const file = new this.constructor(this);

    file.history = history;
    if (utils.isObject(this.stat)) {
  ...
  isFile() {
    ...
    if (this.stat && typeof this.stat.isFile === 'function') {
  ...
  set symlink(val) {
    assert.equal(typeof val, 'string', 'expected file.symlink to be a string');
  ...
  /**
   * Returns true if the given file is a valid vinyl file
   */
  ...
  /**
   * Normalize the given filepath. Override this method to customize behavior.
   * @param {String} `filepath`
   * @return {String} Returns a normalized path with slashes converted to forward slashes and no trailing slash.
   * @api public
   */
  ...
  /**
   * Resolve an absolute filepath. Override this method to customize behavior.
   * @param {String} `filepath`
   * @return {String}
   * @api public
   */
  ...
  static isCloneable(val) {
    return utils.isStream(val) && this.Cloneable.isCloneable(val);
  }
  ...
  /**
   * Returns true if `file.contents` is a readable stream.
   */
  ...
  static inspectStream(stream) {
    return inspectStream(stream);
  }
  ...
  static get builtinProps() {
    return this._builtinProps || (this._builtinProps = new Set([
      '__proto__',
      '_base',
      '_contents',
      '_cwd',
...
function isReadable(val) {
  return utils.isStream(val) && utils.isObject(val._readableState);
}
